...
def save_experiment_as_csv(file_name: str) -> None:
    ...
    for _, session_info in json_output["sessions"].items():
        ...
        for agent_role, agent in session_info["agents"].items():
            if "ID" not in agent["profile_info"]:
                continue

            result_dict[agent["profile_info"]["ID"]] = {
        ...
        for message in session_info["message_history"]:
            if (
            ...
            ):  # Message not generated by user-defined agents
                continue
            else:
                ...
                if (
                    ...
                    result_dict[message["agent_id"]][message["task_id"]] = message[
                        role_label
                    ]

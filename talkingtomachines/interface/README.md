...
| `task_order` | **Yes** | A integer value indicating the order in which the tasks will be executed. If the task order is duplicated across different tasks, then the order of these tasks will be randomized. |
| `is_adapted` | **Yes** | An boolean field indicating if the text from the actual experiment has been adapted. Expected values: `True` or `False`. This field is only used for documentation purposes and does not affect the operation of the platform. |
...
## Video Walkthrough
A video walkthrough of how to populate the prompt template workbook based on an example public goods experiment can be found here: [Video Walkthrough]()
...
| `model_info` | **Yes** | The LLM that will be used in the experiment. Provide any OpenAI-compatible model identifier (including OpenRouter models) or use `hf-inference` when targeting the Hugging Face Inference API. Currently, only the OpenAI-compatible models can accept visual inputs. |
